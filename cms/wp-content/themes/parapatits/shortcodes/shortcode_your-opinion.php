<section id="your-opinion-section" class="your-opinion-section" data-matching-link="#your-opinion-section-link">
	<article class="wrapper">
		<div class="your-opinion-section__container wrapper">
			<h2 class="h2__title your-opinion-section__title wrapper">Wir würden auch Ihre Meinung gerne hören</h2>
			<div data-inviewport="entrance-fade-bottom" class="your-opinion-section__content">
				<p>
					Lassen Sie uns Ihr Feedback zukommen!
				</p>
				<a data-inviewport="entrance-fade-bottom" class="btn btn--red btn--centered-aligned" href="https://g.page/r/CVpT6XCa9L20EAg/review">Feedback schreiben</a>
			</div>
		</div>
	</article>
</section>